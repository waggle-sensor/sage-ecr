...
def client():
    ...
    if os.getenv("TESTING") != "true":
        raise RuntimeError(
            "To prevent accidentally wiping the production database and registry, you must set the env var TESTING=true to run the test suite."
        )
...
def test_auth_header_incomplete(client):
    ...
    assert r.get_json() == {
        "error": "Authorization failed (could not parse Authorization header)"
    }
...
def test_auth_header_invalid_bearer(client):
    r = client.get(
        "/apps/sage/simple/1.2.3", headers={"Authorization": "xyz testuser_token"}
    )
    assert r.status_code == 401
    assert r.get_json() == {
        "error": "Authorization failed (Authorization bearer not supported)"
    }


def test_auth_header_token_not_found(client):
    r = client.get(
        "/apps/sage/simple/1.2.3", headers={"Authorization": "sage doesnotexist"}
    )
    assert r.status_code == 401
...
def test_app_submit_view_delete_lifecycle(client):
    ...
    def assert_matches_app(data):
        ...
        assert (
            data["source"]["url"] == "https://github.com/waggle-sensor/edge-plugins.git"
        )
    ...
    result = must_submit_app_and_get_json(
        client,
        headers=headers,
        app_yaml="""
name: simple
...
""",
    ...
    # start build
    r = client.post(
        "/builds/sagebuildtest/simple/1.0.0?skip_image_push=false", headers=headers
    )
    assert r.status_code == 200
    result = r.get_json()
    assert "error" not in result
...
""",
    ...
    # confirm that image is not in registry before build
    r = requests.head(
        "http://registry:5000/v2/sagebuildtest/alternative_dockerfile/manifests/1.2.3"
    )
    assert r.status_code == 404

    # start build
    r = client.post(
        "/builds/sagebuildtest/alternative_dockerfile/1.2.3?skip_image_push=false",
        headers=headers,
    )
    assert r.status_code == 200
    result = r.get_json()
    ...
    while True:
        r = client.get(
            "/builds/sagebuildtest/alternative_dockerfile/1.2.3", headers=headers
        )
        assert r.status_code == 200
        result = r.get_json()
        assert "error" not in result
...
""",
    ...
    # start build
    r = client.post(
        "/builds/sagebuildtest/failure/1.0.0?skip_image_push=false", headers=headers
    )
    assert r.status_code == 200
    result = r.get_json()
    assert "error" not in result
...
""",
    ...
    assert r.get_json() == {
        "error": "Your account is not approved to perform this action."
    }
...
def test_list_apps_user_visibilty(client):
    ...
    r = client.put(
        f"/permissions/planetmars/simple/1.0",
    ...
    r = client.put(
        f"/permissions/planetmars", data=json.dumps(data), headers=admin_headers
    )

    # submit app as testuser
    must_submit_app_and_get_json(
        client,
        headers=headers_testuser,
        app_yaml="""
name: simple
description: "a simple app"
version: "1.0"
...
""",
    ...
    r = client.get(
        f"/permissions/brothersgrimm/hansel_and_gretel", headers=headers_testuser
    )
    assert r.status_code == 200
    result = r.get_json()
    assert len(result) == 1
    ...
    r = client.get(
        f"/apps/brothersgrimm/hansel_and_gretel/1.0", headers=headers_testuser2
    )
    assert r.status_code == 401
    assert (
        r.data
        == b'{"error":"Not authorized. (User testuser2 does not have permission READ for repository brothersgrimm/hansel_and_gretel)"}\n'
    )
    ...
    result = r.get_json()
    assert not any(
        app["id"] == "brothersgrimm/hansel_and_gretel:1.0" for app in result["data"]
    )
    ...
    r = client.put(
        f"/permissions/brothersgrimm/hansel_and_gretel",
        data=json.dumps(data),
        headers=headers_testuser,
    )
    assert r.status_code == 200
    ...
    r = client.get(
        "/permissions/brothersgrimm/hansel_and_gretel", headers=headers_testuser
    )
    assert r.status_code == 200
    result = r.get_json()
    assert sorted_by_grantee_and_permission(result) == [
        {
            ...
            "granteeType": "GROUP",
            "permission": "READ",
            "resourceName": "brothersgrimm/hansel_and_gretel",
    ...
    r = client.get(
        "/apps/brothersgrimm/hansel_and_gretel/1.0", headers=headers_testuser
    )
    assert r.status_code == 200

    # get public app as another user
    r = client.get(
        "/apps/brothersgrimm/hansel_and_gretel/1.0", headers=headers_testuser2
    )
    assert r.status_code == 200
    ...
    data["operation"] = "delete"
    r = client.put(
        "/permissions/brothersgrimm/hansel_and_gretel",
        data=json.dumps(data),
        headers=headers_testuser,
    )
    assert r.status_code == 200
    ...
    r = client.put(
        f"/permissions/brothersgrimm", data=json.dumps(acl), headers=headers_testuser
    )
    assert r.status_code == 200
    result = r.get_json()
    assert result["added"] == 1
    ...
    assert any(
        n["id"] == "brothersgrimm"
        and n["owner_id"] == "testuser"
        and n["type"] == "namespace"
    ...
    r = client.get(
        f"/permissions/brothersgrimm/hansel_and_gretel", headers=headers_testuser2
    )
    assert r.status_code == 200
    result = r.get_json()
    assert "error" not in result
...
""",
    ...
    for f in file_dicts:
        name = f["name"]
        r = client.get(
            f"/meta-files/sage/example_with_meta_files/1.2.3/{name}", headers=headers
        )
        assert r.status_code == 200
        result = r.get_data()
...
""",
    ...
    r = client.post(
        f"/authz", headers=headers_sage_docker_auth, data=json.dumps(sample_request)
    )
...
def delete_docker_registry_manifest(name, tag):
    ...
    r = requests.head(
        f"http://registry:5000/v2/{name}/manifests/{tag}",
