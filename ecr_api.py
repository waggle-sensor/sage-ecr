...
app_submission_counter = Counter(
    "app_submission_counter",
    "This metric counts the total number of successful app submissions.",
)
build_request_counter = Counter(
    "build_request_counter", "This metric counts the total number of requested builds."
)
...
if config.auth_method == "static":
    ...
    app_authenticator = authenticators.SageAuthenticator(
        url=config.tokenInfoEndpoint, password=config.tokenInfoPassword
    )
else:
...
class ECRAuthMiddleware:
    ...
    def __call__(self, environ, start_response):  # pragma: no cover
        ...
        if len(authHeaderArray) != 2:
            ...
            return ErrorWResponse(
                f"Authorization failed (could not parse Authorization header)",
        ...
        if bearer not in ["sage", "static"]:
            ...
            return ErrorWResponse(
                f"Authorization failed (Authorization bearer not supported)",
                status_code=HTTPStatus.UNAUTHORIZED,
            )(environ, start_response)
        ...
        try:
            ...
            return ErrorWResponse(
                "Token not found", status_code=HTTPStatus.UNAUTHORIZED
            )(environ, start_response)
    ...
    def get_token_info_with_caching(self, token):
        try:
            ...
            app.logger.info(
                "auth middleware: using cached token for %s", token_info.user
            )
            return token_info
        except KeyError:
            pass
        ...
        app.logger.info(
            "auth middleware: updating cached token for %s", token_info.user
        )
...
def preprocess_repository(url, branchOrTag, custom_version, namespace, repository):
    ...
    with tempfile.TemporaryDirectory(dir=temp_dir) as tmpdirname:
        ...
        subprocess.run(
            ["git", "clone", "--recursive", "-b", branchOrTag, url, str(tmpdirname)],
        ...
        if exit_code != 0:
            ...
            raise Exception(
                f"Extracting version number failed (stdout={ stdout_str }, stderr={stderr_str}, tmpdirname={tmpdirname})"
            )
        ...
        if exit_code != 0:
            ...
            if stderr:
                stderr_str = stderr.decode("utf-8")
            raise Exception(
                f"Extracting hash number failed (stdout={ stdout_str }, stderr={stderr_str}, tmpdirname={tmpdirname})"
            )
        ...
        if exit_code != 0:
            ...
            if stderr:
                stderr_str = stderr.decode("utf-8")
            raise Exception(
                f"Creating archive failed (stdout={ stdout_str }, stderr={stderr_str}, tmpdirname={tmpdirname})"
            )
...
def submit_app(
    ...
    if not ok:
        ...
        userHasNamespaceWritePermission = ecr_db.hasPermission(
            "namespace", namespace, "USER", requestUser, "WRITE"
        )
    ...
    if not ok:
        ...
        if userHasNamespaceWritePermission:
            try:
                ...
                raise Exception(
                    f"Could not create repository {namespace}/{repository}: {str(e)}"
                )
        else:
            raise ErrorResponse(
                f"Not authorized to access namespace {namespace}",
                status_code=HTTPStatus.UNAUTHORIZED,
            )

    else:
        # repo exists, check if user has namespace or repo permission
        userHasRepoWritePermission = ecr_db.hasPermission(
            "repository", f"{namespace}/{repository}", "USER", requestUser, "WRITE"
        )
        if (not userHasNamespaceWritePermission) and (not userHasRepoWritePermission):
            raise ErrorResponse(
                f"Not authorized to access repository {namespace}/{repository}",
                status_code=HTTPStatus.UNAUTHORIZED,
            )
    ...
    if version:  # if not, version will be extracted from git repo
        try:
            existing_app, found_app = ecr_db.listApps(
                user=requestUser,
                namespace=namespace,
    ...
    for key in config.required_fields:
        ...
        if type(value).__name__ != expected_type:
            raise Exception(
                f"Field {key} has to be of type {expected_type}, got {type(value).__name__}"
            )
    ...
    extracted_version, git_commit = preprocess_repository(
        url, branch_or_tag, version, namespace, repository
    )
    ...
    if not version:
        ...
        if found_app:
            if existing_app.get("frozen", False) and (not isAdmin):
                raise Exception(
                    f"App {namespace}/{repository}:{version} already exists and is frozen."
                )

            if not force_overwrite:
                raise Exception(
                    f"App {namespace}/{repository}:{version} already exists but is not frozen. Use query force=true to overwrite."
                )
    ...
    dbObject["metadata"] = json.dumps(
        appMetadata
    )  # should at least be an empty dictionary
    ...
    try:
        ecr_db.insertApp(
            col_names_str, values, variables_str, sources_values, resourcesArray
        )
    except Exception as e:
    ...
    try:
        import_meta_files(
            returnObj, namespace=namespace, repository=repository, version=version
        )
    except Exception as e:
...
class Submit(MethodView):
    ...
    def post(self):
        ...
        try:
            ...
            raise ErrorResponse(
                f"{str(e)}", status_code=HTTPStatus.INTERNAL_SERVER_ERROR
            )
...
def import_meta_files(app_obj, namespace=None, repository=None, version=None):
    ...
    file_dicts = [
        {"name": "ecr-icon.jpg", "kind": "thumb"},
        {"name": "ecr-science-image.jpg", "kind": "image"},
        {"name": "ecr-science-description.md", "kind": "science_description"},
    ]
...
class Apps(MethodView):
    ...
    def delete(self, namespace, repository, version):
        ...
        try:
            ...
            app.logger.error(
                "failed to delete app %s/%s:%s by %s: %s",
                namespace,
                repository,
                version,
                requestUser,
            ...
            raise ErrorResponse(
                f"Error deleting app: {e}", status_code=HTTPStatus.INTERNAL_SERVER_ERROR
            )
    ...
    def put(self, namespace, repository, version):
        ...
        if "frozen" in request.args:
            ...
            if not isinstance(frozen, bool):
                raise ErrorResponse(
                    f"frozen is not bool", status_code=HTTPStatus.INTERNAL_SERVER_ERROR
                )

            result = ecr_db.setAppField(
                namespace, repository, version, "frozen", frozen
            )
            result_obj = {"modified": result}
        else:
            raise ErrorResponse(
                f"Not sure what to do", status_code=HTTPStatus.INTERNAL_SERVER_ERROR
            )
...
class AppsGlobal(MethodView):
    ...
    def get(self, namespace=None, repository=None, version=None):
        ...
        try:
            returnList = ecr_db.listApps(
                user=requestUser,
                namespace=namespace,
                repository=repository,
                isAdmin=isAdmin,
                limit=limit,
                continuationToken=continueT,
                filter=filter,
                view=view,
            )
        except Exception as e:
            raise ErrorResponse(
                f"listApps returned: {str(e)}",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )

        return_obj = {"pagination": {}, "data": returnList}
        ...
        if len(returnList) > 0:
            if len(returnList) == limit:
                ...
                return_obj["pagination"]["continuationToken"] = base64.b64encode(
                    str.encode(last_id)
                ).decode()  # b64 is just to make it look cooler
            else:
...
def get_build(requestUser, isAdmin, namespace, repository, version):
    try:
        js = jenkins_server.JenkinsServer(
    ...
    if not ok:
        ...
        return ErrorResponse(
            f"app_spec not found {namespace}/{repository}:{version}",
            status_code=HTTPStatus.NOT_FOUND,
        )
    app_id = app_spec.get("id", "")
    if not app_id:
        return ErrorResponse(
            f"app id not found", status_code=HTTPStatus.INTERNAL_SERVER_ERROR
        )
    ...
    try:
        ...
        raise ErrorResponse(
            f"Could not get build number: {str(e)}",
            status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
        )
...
def build_app(
    requestUser, isAdmin, namespace, repository, version, skip_image_push=False
):
    host = config.jenkins_server
    username = config.jenkins_user
    password = config.jenkins_token
    ...
    try:
        js.createJob(
            app_human_id, app_spec, overwrite=overwrite, skip_image_push=skip_image_push
        )
    except Exception as e:
        raise ErrorResponse(
            f"createJob() returned: {str(e)}",
            status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
        )
    ...
    if not architectures:
        raise ErrorResponse(
            f"architectures not specified in source",
            status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
        )
    ...
    try:
        ...
        raise ErrorResponse(
            f"error inserting build info for {app_id}, {build_name}, {number} , SaveBuildInfo: {str(e)}",
            status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
        )
...
class Builds(MethodView):
    ...
    def get(self, namespace, repository, version):
        ...
        try:
            ...
            raise ErrorResponse(
                f"get_build returned: {type(e).__name__},{str(e)}",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
...
class NamespacesList(MethodView):
    ...
    def put(self):
        ...
        if not requestNamespace:
            raise ErrorResponse(
                f'Field "namespace" missing in request',
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
        ...
        if ok:
            raise ErrorResponse(
                f"Namespace {requestNamespace} already exists",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
        ...
        try:
            ...
            raise ErrorResponse(
                f"Could not create namespace {requestNamespace}: {str(e)}",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
...
class Namespace(MethodView):
    ...
    def get(self, namespace, repository=None):
        ...
        if not namespace:
            raise ErrorResponse(
                f"Namespace not found", status_code=HTTPStatus.INTERNAL_SERVER_ERROR
            )
    ...
    def delete(self, namespace, repository=None):
        ...
        if repo_count > 0:
            raise ErrorResponse(
                f"Namespace is not empty, {repo_count} repositories found",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
...
class RepositoriesList(MethodView):
    def get(self, namespace=None):
        ...
        ecr_db = ecrdb.EcrDB()
        repList = ecr_db.listRepositories(
            user=requestUser, namespace=namespace, isAdmin=isAdmin, filter=filter
        )
...
class Repository(MethodView):
    def get(self, namespace, repository):
        ...
        if not ok:
            raise ErrorResponse(
                f"Repository {namespace}/{repository} not found",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
    ...
    def delete(self, namespace, repository, version=None):
        ...
        if app_count > 0:
            raise ErrorResponse(
                f"Repository {namespace}/{repository} not empty. It contains {app_count} apps",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
...
class Permissions(MethodView):
    ...
    def put(self, namespace, repository=None, version=None):
        ...
        try:
            ...
            raise ErrorResponse(
                f"request.get_json returned: {str(e)}",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
        ...
        if not "operation" in postData:
            raise ErrorResponse(
                f"Field operation missing", status_code=HTTPStatus.INTERNAL_SERVER_ERROR
            )
        ...
        if operation == "add":
            ...
            try:
                result = ecr_db.addPermission(
                    resource_type, resource_name, granteeType, grantee, permission
                )
            except Exception as e:
                raise ErrorResponse(
                    f"ecr_db.addPermissionreturned: {str(e)}",
                    status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
            ...
            if repository:
                ...
                if not ok:
                    raise ErrorResponse(
                        f"No owner found for repository {namespace}/{repository}",
                        status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
                    )
            ...
            try:
                result = ecr_db.deletePermissions(
                    owner,
                    resource_type,
                    resource_name,
                ...
                raise ErrorResponse(
                    f"ecr_db.deletePermissions: {str(e)}",
                    status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
                )
            ...
            raise ErrorResponse(
                f"Operation can only be add or delete.",
                status_code=HTTPStatus.INTERNAL_SERVER_ERROR,
            )
...
class AuthZ(MethodView):
    ...
    def post(self):
        ...
        if not "ecr_authz_introspection" in user_scopes_array:
            print(
                f"AuthZ rejected, user is not allowed to ask, got user_scopes: {user_scopes}",
                file=sys.stderr,
            )
            raise ErrorResponse(
                "User does not have permission to access the introspection",
                status_code=HTTPStatus.UNAUTHORIZED,
            )
        ...
        if postData.get("type", "") != "repository":
            ...
            raise ErrorResponse(
                "This type is not supported", status_code=HTTPStatus.UNAUTHORIZED
            )
        ...
        perm_table = {"pull": "READ", "push": "WRITE"}
        ...
        for act in actions:
            ...
            if not asking_permission:
                ...
                raise ErrorResponse(
                    f"Action {act} unknown", status_code=HTTPStatus.UNAUTHORIZED
                )
        ...
        if len(approved_permissions) == 0:
            raise ErrorResponse(
                f"No actions approved", status_code=HTTPStatus.UNAUTHORIZED
            )
...
app.add_url_rule(
    "/submit", view_func=Submit.as_view("submitAPI"), strict_slashes=False
)  #  this is a shortcut, replacing POST to /apps/<string:namespace>/<string:repository>

app.add_url_rule(
    "/apps", view_func=AppsGlobal.as_view("appsGlobal"), strict_slashes=False
)
app.add_url_rule(
    "/apps/<string:namespace>",
    view_func=AppsGlobal.as_view("AppsGlobal_namespaces"),
    strict_slashes=False,
)
app.add_url_rule(
    "/apps/<string:namespace>/<string:repository>",
    view_func=AppsGlobal.as_view("AppsGlobal_repo"),
    strict_slashes=False,
)
app.add_url_rule(
    "/apps/<string:namespace>/<string:repository>/<string:version>",
    view_func=Apps.as_view("appsAPI"),
)

app.add_url_rule(
    "/namespaces",
    view_func=NamespacesList.as_view("namespacesListAPI"),
    strict_slashes=False,
)
app.add_url_rule(
    "/namespaces/<string:namespace>", view_func=Namespace.as_view("namespacesAPI")
)

app.add_url_rule(
    "/repositories",
    view_func=RepositoriesList.as_view("repositoriesListAPI"),
    strict_slashes=False,
)
app.add_url_rule(
    "/repositories/<string:namespace>",
    view_func=RepositoriesList.as_view("repositoryAPI_namespaced"),
)
app.add_url_rule(
    "/repositories/<string:namespace>/<string:repository>",
    view_func=Repository.as_view("repositoryAPI"),
)

app.add_url_rule(
    "/permissions/<string:namespace>/<string:repository>",
    view_func=Permissions.as_view("permissionsAPI_2"),
    methods=["GET", "PUT", "DELETE"],
    strict_slashes=False,
)
app.add_url_rule(
    "/permissions/<string:namespace>",
    view_func=Permissions.as_view("permissionsAPI_3"),
    methods=["GET", "PUT", "DELETE"],
    strict_slashes=False,
)

app.add_url_rule(
    "/builds/<string:namespace>/<string:repository>/<string:version>",
    view_func=Builds.as_view("buildsAPI"),
)

app.add_url_rule(
    "/archive/<string:namespace>/<string:repository>/<string:version>.tgz",
    view_func=CodeArchive.as_view("codeArchiveAPI"),
)
...
app.add_url_rule(
    "/meta-files/<string:namespace>/<string:name>/<string:version>/<string:file_name>",
    view_func=MetaFiles.as_view("metaFilesAPI"),
    strict_slashes=False,
)
